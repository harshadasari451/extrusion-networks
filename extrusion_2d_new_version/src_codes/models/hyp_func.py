...
class Unet_blocks_hyp(nn.Module):
    ...
    def __init__(self, f_size = 1, z_dim = 64, out_size=32, in_size=32):
        ...
        self.hyp_linear = nn.Linear(4,8).cuda()
    ...
    def forward(self, z, pde_inputs):
        ...
        pde_features = torch.relu(pde_features)  # Non-linear activation after linear layer

        z_pde_features = torch.cat((z, pde_features), dim=0)

        h_in = torch.matmul(z_pde_features, self.w2) + self.b2
        h_in = torch.relu(h_in)  # Activation here
...
class Unet_upconv_hyp(nn.Module):
    ...
    def forward(self, z, pde_inputs):
        ...
        h_in = torch.matmul(z_pde_features, self.w2) + self.b2
        h_in = torch.relu(h_in)  # Activation heres
        h_in = h_in.view(self.in_size, self.z_dim)
