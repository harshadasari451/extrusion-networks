...
from .hyp_func import Unet_blocks_hyp



class upconv_Embedding(nn.Module):

    def __init__(self, z_num, z_dim,unet_1d_weights, unet_1d_bias, device):
    ...
    def forward(self, hyper_net, pde_inputs):
        ...
        for i in range(h):
            ...
            for j in range(k):
                ...
                for l in range(2):
                    z_idx = i * (k * 2) + j * 2 + l
                    w.append(hyper_net(self.z_list[z_idx], pde_inputs))  
...
class PrimaryNetwork(nn.Module):
    ...
    def __init__(self, z_dim=64, unet_1d_weights_path = None, device = None):
        ...
        while i <= 25:
            if i in (10,14,18,22):
                upconv_module = upconv_Embedding(self.zs_size[i], self.z_dim, self.unet_1d_weights_list[i], self.unet_1d_weights_list[i+1], device=self.device)
    ...
    def forward(self, x, pde_inputs):
        ...
        # decoder_part
        w, b = self.zs_upconv[0](self.unet_block_hope, pde_inputs)
        ...
        w, b = self.zs_upconv[1](self.unet_block_hope, pde_inputs)
        ...
        w, b = self.zs_upconv[2](self.unet_block_hope, pde_inputs)
        ...
        w, b = self.zs_upconv[3](self.unet_block_hope, pde_inputs)
